...
model HeatingOffice "Office heating with TABS"
  ...
  extends
    IDEAS.Thermal.HeatingSystems.Interfaces.Partial_HydraulicHeatingSystem(
       final emissionType=EmissionType.FloorHeating, nLoads=1);

  IDEAS.Thermal.Components.BaseClasses.Pump[nZones] pumpRad(
     each medium=medium,
     each useInput=true,
     m_flowNom=m_flowNom,
     each m=1)
  ...
  IDEAS.Thermal.Components.Emission.EmbeddedPipe[nZones] emission(each
       medium=medium, m_flowMin=m_flowNom)
  ...
  replaceable IDEAS.Thermal.Control.Ctrl_Heating ctrl_Heating(
     heatingCurve(timeFilter=timeFilter),
     TSupNom=TSupNom,
     dTSupRetNom=dTSupRetNom,
     THeaterSet(start=293.15)) constrainedby
    IDEAS.Thermal.Control.Interfaces.Partial_Ctrl_Heating(
     heatingCurve(timeFilter=timeFilter),
     TSupNom=TSupNom,
  ...
  IDEAS.Thermal.Components.BaseClasses.Thermostatic3WayValve idealMixer(mFlowMin=
         0.01)
    annotation (Placement(transformation(extent={{28,22},{50,46}})));
  IDEAS.Thermal.Components.BaseClasses.Pipe_Insulated pipeReturn(
     medium=medium,
     m=1,
     UA=10)
    annotation (Placement(transformation(extent={{4,-28},{-16,-36}})));

  IDEAS.Thermal.Components.BaseClasses.Pipe_Insulated pipeSupply(
     medium=medium,
     m=1,
     UA=10)
    annotation (Placement(transformation(extent={{-16,30},{4,38}})));
  IDEAS.Thermal.Components.BaseClasses.Pipe_Insulated[nZones] pipeReturnEmission(
     each medium=medium,
     each m=1,
     each UA=10)
    annotation (Placement(transformation(extent={{116,-28},{96,-36}})));
  IDEAS.Thermal.Components.BaseClasses.AbsolutePressure absolutePressure
    annotation (Placement(transformation(extent={{-94,-42},{-114,-22}})));
equation
  QHeatTotal = -sum(emission.heatPortEmb.Q_flow);
